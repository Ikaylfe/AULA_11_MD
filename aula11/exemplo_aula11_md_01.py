...
try:
    ...
    plt.hist(array_roubo_veiculo, bins=394)
    plt.axvline(media_roubo_veiculo, color = 'red', linewidth=1)
    plt.axvline(mediana_roubo_veiculo, color = 'green', linewidth=1)

    # P/ printar as faixas de intervalo do Histograma
    contagens, limites = np.histogram(array_roubo_veiculo, bins=394)
    ...
    desvio_padrao_var=np.std(array_roubo_veiculo)
    desvio_padrao_var = (desvio_padrao_var / media_roubo_veiculo) * 100
    coef_var = desvio_padrao_var / media_roubo_veiculo
    ...
    plt.text(0.1, 0.0, f'Amplitude Total: {amplitude :.2f}', fontsize=12)

    plt.text(0.5, 0.9, f'Assimetria: {assimetria :.2f}', fontsize=12)
    plt.text(0.5, 0.8, f'Curtose: {curtose :.2f}', fontsize=12)
    plt.text(0.5, 0.7, f'Variância: {variancia :.2f}', fontsize=12)
    plt.text(0.5, 0.5, f'Distância variância: {distancia_var_media :.2f}', fontsize=12)
    plt.text(0.5, 0.4, f'Desvio padrão: {desvio_padrao_var :.2f}', fontsize=12)
    plt.text(0.5, 0.3, f'Coeficiencia: {coef_var :.2f}', fontsize=12)
...
#     #É A NORMALIZAÇÃO DA VARIÂNCIA. pOR ISSO É O MAIS FÁCIL A INTERPRETAÇÃO
#     #APRESENTAR O QUANTO OS DADOS  ESTÃO AFASTADOS DA MÉDIA;
#     desvio_padrao=np.std(array_roubo_veiculo)

#     desvio_padrao_var = (desvio_padrao / media_roubo_veiculo) * 100
...
#     #É a magnitude(tamanho do desvio) do desvio padrão em relação a média.
#     coef_var = desvio_padrao / media_roubo_veiculo
...
#     print(f'Distância entre média e variância = {distancia_var_media :.2f}%')
#     print(f'Desvio padrao= {desvio_padrao :.2f}')
#     print(f'Coeficiência de variação = {coef_var :.2f}')
